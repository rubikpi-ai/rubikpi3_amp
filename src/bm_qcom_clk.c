...
/*
 * QCOM RCG2 fractional programming (common):
 *  - hid divider encoded as (pre_div-1) in CFG[12:8]
 *  - M = m
 *  - N = n
 *  - D = ~(n - m)
 *  - then CMD.UPDATE
 *
 * For bypass: m=n=0 => M=N=D=0
 */
...
/*
 * Clone of Linux geni_se_clk_freq_match() behavior:
 * - choose entry minimizing delta = req - (tbl[i] / ceil(tbl[i]/req))
 * - exact==1 requires delta==0 else fail
 */
